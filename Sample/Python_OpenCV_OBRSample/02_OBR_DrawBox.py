...
# 이미지를 읽어옵니다
img1 = cv2.imread('box.png', 0)          # 쿼리 이미지 (객체 이미지)
img2 = cv2.imread('box_in_scene.png', 0) # 트레인 이미지 (장면 이미지)
...
# 매칭을 그립니다
draw_params = dict(matchColor=(0, 255, 0),  # 매칭된 선의 색상
                   singlePointColor=(255, 0, 0),  # 키포인트의 색상
                   matchesMask=matchesMask,  # 매칭 마스크 적용
                   flags=0)

img3 = cv2.drawMatchesKnn(img1, kp1, img2, kp2, matches, None, **draw_params)

# img1에서 img2로 객체를 찾습니다
if len(good_matches) > 4:  # 호모그래피를 찾기 위해서는 최소 4개의 점이 필요합니다
    ...
    # 최종 매칭과 검출된 사각형을 화면에 표시합니다
    plt.imshow(img2, 'gray')
    plt.show()
else:
    print("충분한 매칭이 발견되지 않았습니다 - {}/{}".format(len(good_matches), len(matches)))
